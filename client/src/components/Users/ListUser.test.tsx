...
const mockListUsersFilled: any[] = [
  { usuario_id: 1, name: "Javi" },
...
it("renders all List users correctly", () => {
  ...
  for (let i = 1; i < mockListUsersFilled.length; i++) {
    const idText = screen.getByTestId(
      `id ${mockListUsersFilled[i - 1].usuario_id}`
    );
    expect(idText.tagName).toBe("TD");
    expect(idText.textContent).toBe(
      mockListUsersFilled[i - 1].usuario_id.toString()
    );

    const nameText = screen.getByTestId(
      `name ${mockListUsersFilled[i - 1].usuario_id}`
    );
    expect(nameText.tagName).toBe("TD");
    expect(nameText.textContent).toBe(mockListUsersFilled[i - 1].name);

    const currentButton = screen.getByTestId(
      `button ${mockListUsersFilled[i - 1].usuario_id}`
    );
    expect(currentButton.tagName).toBe("BUTTON");
    expect(currentButton.textContent).toBe(
      mockListUsersFilled[i - 1].name + " connections"
    );
  }
});
