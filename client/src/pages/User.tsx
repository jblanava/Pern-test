    ...
    const onSubmitForm = (e: any, name: string) => {
        ...
        .then((res) => {
            if(res.status >=200 && res.status < 300){
                ...
                }).catch((reason) => console.error("Create user res Json promise rejected : " + reason));
            }
        })
        .catch((reason) => console.error("Create user promise rejected : " + reason));
      ...
      const getUsers = () => {
        fetch("http://localhost:3000/users").then(res => {
          res.json()
            .then((res) => {
              setUsers(res);
            }).catch((reason) => console.error("All users promise rejected : " + reason));
        });
      };
