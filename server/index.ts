...
app.post("/users", (req: any, res: any) => {
  const { name } = req.body;
  psqlPool
    .query("INSERT INTO usuario (name) VALUES($1) RETURNING * ", [name])
...
app.get("/users", async (req: any, res: any) => {
  try {
    const allUsers = await psqlPool.query("SELECT * FROM usuario");
...
app.get("/connections", async (_req: any, res: any) => {
  try {
    const getAllConnections = await psqlPool.query("SELECT * FROM connection");
